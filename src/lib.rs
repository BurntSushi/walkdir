...
On supported Unix platforms, traversal opens children with `openat`; Windows
uses `NtOpenFile` for the same parent-relative operation. This avoids platform
path-length limits and prevents symlink swaps between listing and descending
while that parent remains open.
